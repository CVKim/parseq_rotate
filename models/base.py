...
class BaseSystem(nn.Module, ABC):
    ...
    def _eval_step(self, batch, validation: bool):
        ...
        if validation:
            ...
            logits = self.forward_loss(images)
            loss = loss_numel = (
    ...
    def train_acc(self, logits, labels):
        correct = 0
        total = 0
        ned = 0
        confidence = 0
    ...
    @staticmethod
    def _aggregate_results(outputs):
        if not outputs:
            return 0.0, 0.0, 0.0
        total_loss = 0
        total_loss_numel = 0
    ...
    def validation_epoch_end(self, outputs):
        ...
        return dict(val_accuracy=100 * acc, val_NED=100 * ned, val_loss=loss, acc=acc)
...
class CrossEntropySystem(BaseSystem):
    ...
    def __init__(
        ...
        super().__init__(tokenizer, charset_test, lr, warmup_pct, weight_decay)
    ...
    def forward_logits_loss(
        ...
        logits = self.forward_loss(images, max_len)
