...
class PARSeq(CrossEntropySystem):
    ...
    def __init__(
        ...
        (
            ...
            self.dec_mlp_ratio,
            self.dec_num_heads,
        ) = (dropout, refine_iters, perm_num, dec_depth, dec_mlp_ratio, dec_num_heads)
        self.decode_ar, self.perm_mirrored, self.perm_forward = (
            decode_ar,
            perm_mirrored,
            perm_forward,
        )
        self.enc_depth, self.enc_mlp_ratio, self.enc_num_heads, self.embed_dim = (
            enc_depth,
            enc_mlp_ratio,
            enc_num_heads,
            embed_dim,
        )
        self.max_label_length = max_label_len
        ...
        decoder_layer = DecoderLayer(
            self.embed_dim,
            self.dec_num_heads,
            self.embed_dim * self.dec_mlp_ratio,
            self.dropout,
        )
        self.decoder = Decoder(
            decoder_layer, num_layers=self.dec_depth, norm=nn.LayerNorm(self.embed_dim)
        )

        # Perm/attn mask stuff
        self.rng = np.random.default_rng()
        self.max_gen_perms = self.perm_num // 2 if self.perm_mirrored else self.perm_num
        self.perm_forward = self.perm_forward
        self.perm_mirrored = self.perm_mirrored
        ...
        self.pos_queries = nn.Parameter(
            torch.Tensor(1, self.max_label_length + 1, self.embed_dim)
        )
            ...
            for i in range(num_steps):
                ...
                if j < num_steps:
                    ...
                    if testing and (tgt_in == self.eos_id).any(dim=-1).all():
                        # print('break the loop: system line 121')
                        break
    ...
    def forward_export(self, images):
        ...
        if self.decode_ar:
            ...
            for i in range(num_steps):
                ...
                if j < num_steps:
                    # greedy decode. add the next token index to the target input
                    # tgt_in[:, j] = p_i.squeeze().argmax(-1)  #original and torchscript work
                    tgt_in[:, j] = p_i.squeeze(1).argmax(
                        -1
                    )  # for onnxruntime and TRT with 8.4.
                    # tgt_in[:, j] = p_i.argmax(-1).squeeze()
                    # Efficient batch decoding: If all output words have at least one EOS token, end decoding.
                    # if testing and (tgt_in == self.eos_id).any(dim=-1).all():         # change for export
                    #     # print('break the loop: system line 121')
                    #     break
            logits = torch.cat(logits, dim=1)
        else:
    ...
    def gen_tgt_perms(self, tgt):
        ...
        This works because the same attention mask can be used for the shorter sequences because of the padding mask.
        """
    ...
    def forward_export_one(self, images):
        max_length = self.max_label_length
        bs = images.shape[0]
        # +1 for <eos> at end of sequence.
        num_steps = max_length + 1
        ...
        # Query positions up to `num_steps`
        pos_queries = self.pos_queries[:, :num_steps].expand(bs, -1, -1)
        # pos_queries = self.pos_queries[:, :num_steps,:].expand(images.shape[0], -1, -1)
        # Special case for the forward permutation. Faster than using `generate_attn_masks()`
        tgt_mask = query_mask = torch.triu(
            torch.full((num_steps, num_steps), float("-inf"), device=self._device), 1
        )
